let dbSession,deducirMensajeError,generateRandomString,validarEmail,enviarEmail,obtenerDataBase;_d5c‍.x([["verificarAcceso",()=>verificarAcceso],["obtenerDatosSocio",()=>obtenerDatosSocio],["cambiarClaveAcceso",()=>cambiarClaveAcceso],["resetearClaveAcceso",()=>resetearClaveAcceso]]);_d5c‍.w("../../../pool",[["default",["dbSession"],function(v){dbSession=v}]]);_d5c‍.w("../../../utils",[["deducirMensajeError",["deducirMensajeError"],function(v){deducirMensajeError=v}],["generateRandomString",["generateRandomString"],function(v){generateRandomString=v}],["validarEmail",["validarEmail"],function(v){validarEmail=v}],["enviarEmail",["enviarEmail"],function(v){enviarEmail=v}],["obtenerDataBase",["obtenerDataBase"],function(v){obtenerDataBase=v}]]);
...
const validarSocio = async (pool, login, clave) => {     
    try{            
        ...
        const { rows } = await pool.query("SELECT * FROM appsocios.login_socios($1,$2) AS (clp_codigo INTEGER, clp_descri TEXT, clp_id TEXT, clp_cedruc TEXT)", [login.trim(), clave.trim()]);
        if(rows.length === 0){
        ...
        usuario.clp_descri = deducirMensajeError(error);
        return usuario;
    }    
}
    ...
    res.send({error:'N', mensaje:'', objetos:respuesta.objetos});
       ...
       const obtenerDatosSocio = async (pool, login, bus) => {
    try{        
       ...
       const cambiarClaveAcceso = async (req, res) => {
